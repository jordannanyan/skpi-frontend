...
@section('content')
    <div class="mb-3">
        <a href="{{ route('prodi.kerja_praktek.create') }}" class="btn btn-primary">
                ...
                <tbody>
                    @foreach ($data as $index => $kp)
                        <tr>
                            ...
                            <td>
                                <a href="{{ route('prodi.kerja_praktek.edit', $kp['id_kerja_praktek']) }}" class="btn btn-warning btn-sm">Edit</a>
                                <form action="{{ route('prodi.kerja_praktek.destroy', $kp['id_kerja_praktek']) }}" method="POST" class="d-inline" onsubmit="return confirm('Yakin ingin menghapus?')">
                                    @csrf
                                    @method('DELETE')
