...
@section('content')
    ...
    @endif
    <div class="mb-3">
        <a href="{{ route('mahasiswa.cpl_skor.create') }}" class="btn btn-primary">
                ...
                <tbody>
                    @foreach ($data as $index => $item)
                        <tr>
                            ...
                            <td>{{ $item['skor_cpl'] }}</td>
                            <td>
                                <a href="{{ route('mahasiswa.cpl_skor.edit', $item['id_cpl_skor']) }}" class="btn btn-sm btn-warning">
                                    <i class="fas fa-edit"></i> Edit
                                </a>
                                <form action="{{ route('mahasiswa.cpl_skor.destroy', $item['id_cpl_skor']) }}" method="POST" style="display:inline-block">
                                    @csrf
                                    @method('DELETE')
