...
@section('content')
    <div class="card">
        <div class="card-body">
            <form action="{{ route('mahasiswa.isi_capaian.update', $isiCapaian['id_capaian']) }}" method="POST">
                @csrf
                @method('PUT')
                ...
                <a href="{{ route('mahasiswa.isi_capaian.index') }}" class="btn btn-secondary">Batal</a>
            </form>
        </div>
    </div>
@stop
